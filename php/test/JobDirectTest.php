...
{
    public function test_direct_list_job(): void
    {
        $setup = job_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-job", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function job_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new FreeJobsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
