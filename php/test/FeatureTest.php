...
class FtCtrl extends FreeJobsControl
{
...
class FtClient
{
    ...
    private FreeJobsUtility $utility;

    public function __construct(FreeJobsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeJobsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeJobsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeJobsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeJobsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeJobsConfig::make_feature($name);
    ...
    private static function buildUrl(FreeJobsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeJobsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeJobsResponse($fetched);
        }
    ...
    private function fail(FreeJobsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeJobsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
