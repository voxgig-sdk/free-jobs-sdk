...
class JobEntity
{
    ...
    {
        if ($args) {
            $this->_data = FreeJobsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FreeJobsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    /**
     * List Job items matching the given filter.
     *
     * @param JobListMatch|array|null $reqmatch Match filter (any subset
     *   of Job fields) as an assoc-array; JobListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Job[]|array A list of Job items as assoc-arrays at
     *   the SDK boundary; throws FreeJobsError on failure (item-5 convention).
     */
