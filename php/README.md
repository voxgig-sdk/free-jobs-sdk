...
The PHP SDK for the FreeJobs API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($jobs as $record) {
        ...
        echo $item["id"] . " " . $item["application_url"] . "\n";
    }
} catch (\Throwable $err) {
...
$job = $client->Job()->list();
print_r(array_map(fn($item) => $item->data_get(), $job));
```
...
```
php/
├── freejobs_sdk.php          -- Main SDK class
...
The main class (`freejobs_sdk.php`) exports the SDK class
