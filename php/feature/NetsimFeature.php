...
class FreeJobsNetsimFeature extends FreeJobsBaseFeature
{
    ...
    private function _simulate(FreeJobsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreeJobsContext $ctx, array $applied): void
    {
        $client = $this->client;
