...
(0, node_test_1.describe)('JobDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-job', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-job', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.FREE_JOBS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FreeJobsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['FREE_JOBS_TEST_JOB_ENTID'];
    ...
    const client = new __1.FreeJobsSDK({
        base: 'http://localhost:8080',
