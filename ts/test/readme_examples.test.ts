...
//     (\`new FreeJobsSDK(...)\` / \`FreeJobsSDK.test(...)\` become
//     \`FreeJobsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"job":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeJobsSDK(...)\` and any
// \`FreeJobsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeJobsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeJobsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
