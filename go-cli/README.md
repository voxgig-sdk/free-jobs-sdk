...
boru-driven command-line client **and** interactive REPL for the FreeJobs
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-jobs-cli)
make build
...
./free-jobs-cli list job

# 5. Override the API base URL for a single call
FREE_JOBS_BASE=https://api.example.com ./free-jobs-cli list job
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-jobs-cli
   ```
...
Run with no arguments to open a REPL (prompt `free-jobs>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `job`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-jobs-cli`. |
...
(`./free-jobs-cli <expr>`) and interactively (the REPL), and expressions compose the
