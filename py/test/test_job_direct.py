...
class TestJobDirect:
    ...
    def test_should_direct_list_job(self):
        setup = _job_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-job", "live" if setup["live"] else "unit")
        if _skip:
