...
class FreeJobsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeJobsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeJobsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeJobsTestRunner.getenv("FREE_JOBS_TEST_LIVE")
        override = FreeJobsTestRunner.getenv("FREE_JOBS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeJobsTestRunner.getenv(key)
        ...
        explain = FreeJobsTestRunner.getenv("FREE_JOBS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_JOBS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeJobsTestRunner._test_control is not None:
            return FreeJobsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeJobsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeJobsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeJobsTestRunner.load_test_control()
...
def load_env_local():
    FreeJobsTestRunner.load_env_local()


def env_override(m):
    return FreeJobsTestRunner.env_override(m)


def entity_data(v):
    return FreeJobsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeJobsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeJobsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeJobsTestRunner.load_test_control()


def live_client_options():
    return FreeJobsTestRunner.live_client_options()


def live_delay_ms():
    return FreeJobsTestRunner.live_delay_ms()
