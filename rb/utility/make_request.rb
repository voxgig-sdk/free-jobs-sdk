...
module FreeJobsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FreeJobsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeJobsResponse.new(fetched)
    else
