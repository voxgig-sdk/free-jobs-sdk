...
class JobEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FreeJobsSDK.test(seed, nil)
    seen = base.Job(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Job(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FREE_JOBS_TEST_JOB_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    job_ref01_list_result = job_ref01_ent.list(job_ref01_match, nil)
...
def job_basic_setup(extra)
  ...
  entid_env_raw = ENV["FREE_JOBS_TEST_JOB_ENTID"]
