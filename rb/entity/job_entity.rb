...
class JobEntity
  ...
  def data_set(args)
    if args
      @_data = FreeJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [JobListMatch, Hash, nil] match filter (any subset of
  #   Job fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Job>, Array] the matching Job items; raises FreeJobsError on failure
