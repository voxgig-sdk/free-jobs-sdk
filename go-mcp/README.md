...
MCP server exposing the FreeJobs SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `free-jobs_list` | `entity`, optional `query` map | First page of records as JSON |
| `free-jobs_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `free-jobs_list` and
`free-jobs_load` tools become available in new sessions.
