...
[MCP](https://modelcontextprotocol.io) server exposing the FreeJobs SDK as
two agent tools — `free-jobs_list` and `free-jobs_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-jobs-mcp)
make build

# 2. Provide credentials via the environment
export FREE_JOBS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-jobs \
  -- /absolute/path/to/free-jobs-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-jobs-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-jobs_list: first page of records
{ "entity": "job" }
{ "entity": "job", "query": { } }

// free-jobs_load: one record by id
{ "entity": "job", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-jobs-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-jobs \
     -- "$PWD"/dist/*/free-jobs-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-jobs_list` and `free-jobs_load` tools now appear
   in new sessions. Ask the agent to *"list job using free-jobs"*
   and it calls `free-jobs_list` with `{"entity":"job"}`.
...
```sh
export FREE_JOBS_APIKEY=sk_live_xxx            # API key
export FREE_JOBS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-jobs_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-jobs_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_JOBS_APIKEY` | API key sent with every request. |
| `FREE_JOBS_BASE` | Optional override of the API base URL. |
...
```sh
./free-jobs-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-jobs_load","arguments":{"entity":"job","query":{"id":1}}}}'
```
