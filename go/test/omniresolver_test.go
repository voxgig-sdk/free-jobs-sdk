...
import (
	...
	omni "github.com/voxgig-sdk/free-jobs-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeJobsError); is {
				out := map[string]any{
					"name":    "FreeJobsError",
...
func omniCtx(arg any, client *sdk.FreeJobsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeJobsSDK); is {
			client = live
