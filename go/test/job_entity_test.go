...
func TestJobEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Job(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Job(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_JOBS_TEST_JOB_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		jobRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.job")))
...
func jobBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_JOBS_TEST_JOB_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_JOBS_TEST_JOB_ENTID"])
