...
The Free Jobs API provides access to a comprehensive database of job listings across various industries.

Learn more about Free Jobs API at [joinrise.io](https://joinrise.io).
...
$jobs = $client->Job()->list();
print_r(array_map(fn($item) => $item->data_get(), $jobs));
```
